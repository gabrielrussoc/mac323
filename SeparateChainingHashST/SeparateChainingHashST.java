/* Exercicio 3.4.18 - SeparateChainingHashST (Algs 4)
...
public class SeparateChainingHashST<Key, Value> {
    ...
    private int K;                                // maximum average of probes
    ...
    // construtor novo que define o tamanho medio maximo das chains K
    public SeparateChainingHashST(int M, int K) {
        this.M = M;
        this.K = K;
        this.lgM = 31 - Integer.numberOfLeadingZeros(M);
        st = (SequentialSearchST<Key, Value>[]) new SequentialSearchST[M];
        for (int i = 0; i < M; i++)
            st[i] = new SequentialSearchST<Key, Value>();
    }

    // Metodo que retorna o tamanho da chain de cada balde
    public int[] getSizes() {
        int[] v = new int[M];
        for (int i = 0; i < M; i++)
            v[i] = st[i].size();
    ...
    // modificado para atualizar o lgM
    ...
    // modificado para usar a estrategia de manter um primo maior q M
     ...
     * @throws NullPointerException if <tt>key</tt> is <tt>null</tt>
     */
    // modificado para dar resize se a media das chains ficar maior que K
    ...
    public static void main(String[] args) { 
        ...
        // print chain size for each bucket
        int v[] = st.getSizes();
